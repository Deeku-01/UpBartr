...
const parseIntParam = (param, defaultValue) => {
...
router.post('/', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        const { skillRequestId, message, proposedTimeline, portfolio, experience, whyChooseMe } = req.body;
...
router.get('/my-applications', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
...
router.get('/received', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        const { skillRequestId } = req.query;
        const status = parseQueryParam(req.query.status, 'ALL');
        const page = parseIntParam(req.query.page, 1);
        const limit = parseIntParam(req.query.limit, 10);
        const skip = (page - 1) * limit;
...
// @ts-ignore
router.get('/:applicationId', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
...
// @ts-ignore
router.put('/:applicationId', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        const { applicationId } = req.params;
        const { message, proposedTimeline, portfolio, experience, whyChooseMe } = req.body;
...
// @ts-ignore
router.delete('/:applicationId', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        const { applicationId } = req.params;
        // Check if application exists and user owns it
        const existingApplication = await db.application.findFirst({
...
// @ts-ignore
router.put('/:applicationId/respond', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        const { applicationId } = req.params;
...
// @ts-ignore
router.put('/:applicationId/complete', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        const { applicationId } = req.params;
        const application = await db.application.findFirst({
            where: { id: applicationId },
...
router.get('/stats/dashboard', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        const [totalApplicationsSubmitted, pendingApplications, acceptedApplications, completedApplications, applicationsReceived, pendingReceivedApplications] = await Promise.all([
